...
const createUser = async (email, password) => {
    try {
        ...
        const userRes = await pool.query("INSERT INTO users(email,password) VALUES ($1, $2) RETURNING *", [email, hashPassword]);
        const cartRes = await pool.query("insert into cart(user_id) values ($1) returning *", [userRes.rows[0].id]);
...
const addProductToUserCartDb = async (cartData) => {
