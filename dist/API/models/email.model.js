...
const sendVerificationEmailDB = async (email) => {
    try {
        const isEmailRegistered = await pool.query("SELECT * FROM users WHERE email = $1", [email]);
        ...
        const emailVerificationToken = await pool.query(emailVerificationTokenQuery, [userId]);
...
const selectAndSendNewsletter = async () => {
    try {
        ...
        await Promise.all(subscribedEmails.rows.forEach(async (email) => {
            await transporter.sendMail({
                from: process.env.NOREPLYEMAIL,
                to: email,
                subject: "Newsletter",
