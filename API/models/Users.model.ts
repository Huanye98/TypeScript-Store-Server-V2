...
const addProductToUserCartDb = async (cartData:cartData) => {
...
const removeProductFromUserCartDb = async (
  product_id: number,
  quantity: number,
  user_id: number
) => {
...
const userGetTheirData = async (id: number) => {
  ...
  `;
    ...
    const cart = response.rows[0].cart_items as CartItem[] || [];
    const cartPrice = Object.values(cart).reduce(
